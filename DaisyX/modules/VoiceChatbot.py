...
@LYCIA.on_message(filters.command("hermione"))
async def Lycia(_, message):
    if len(message.command) < 2:
        await message.reply_text("Hermione AI Voice Chatbot")
        return
    text = message.text.split(None, 1)[1]
    lycia = text.replace(" ", "%20")
    m = await message.reply_text("Hermione Is Best...")
    try:
        L = await fetch(
            f"https://api.affiliateplus.xyz/api/chatbot?message={lycia}&botname=Hermione&ownername=percyjackson&user=1"
        )
        chatbot = L["message"]
